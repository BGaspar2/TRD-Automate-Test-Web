...
export class CheckoutPage {
    ...
    constructor(page) {
        ...
        this.inputName = page.locator('input[name="name"], #name, input[placeholder*="Nombre" i], input[placeholder*="nombre" i]').first();
        this.inputLastName = page.locator('input[name="lastName"], #lastName, input[placeholder*="Apellido" i], input[placeholder*="apellido" i]').first();
        this.inputEmail = page.locator('input[name="email"], input[type="email"], input[placeholder*="email" i], input[placeholder*="correo" i]').first();
        this.inputPhoneCustomer = page.locator('.PhoneNumber input, input[name="phone"], input[type="tel"], input[placeholder*="Teléfono" i], input[placeholder*="telefono" i]').first();
        this.inputDocument = page.locator('#document, input[name="document"], input[placeholder*="Documento" i], input[placeholder*="DNI" i], input[placeholder*="Cédula" i], input[placeholder*="Cedula" i]').first();
    ...
    /**
     * Completa el modal o formulario para agregar una nueva tarjeta de débito/crédito en Ecuador
     * @param {object} cardData Datos de la tarjeta
     */
    async agregarNuevaTarjeta(cardData) {
        console.log("Iniciando proceso para agregar nueva tarjeta en Ecuador...");
        ...
        for (let intento = 1; intento <= 5; intento++) {
            ...
            if (modalAbierto) {
                console.log("✅ Modal de tarjeta abierto exitosamente en Ecuador.");
                break;
            }
        }
        ...
        console.log(`Llenando formulario de tarjeta en Ecuador: Num=${numAEnviar}, Mes=${mesAEnviar}, Año=${anioAEnviar}, CVV=${cvvAEnviar}...`);
        ...
        console.log("Guardando tarjeta en Ecuador...");
        ...
        await this.page.waitForTimeout(2000);
        console.log("✅ Proceso de adición de tarjeta finalizado en Ecuador.");
    }
     ...
     * - 'Efectivo (Con cambio)'
     * @param {'tarjeta' | 'punto_venta' | 'efectivo_exacto' | 'efectivo_cambio' | string} metodoPago
     * @param {string|number|object} [montoCambioOCard]
     */
    async seleccionarMetodoPago(metodoPago = 'Punto de Venta', montoCambioOCard = null) {
        ...
        const esPuntoVenta = !esTarjeta && (metodoLower.includes('punto') || metodoLower.includes('pos') || metodoLower.includes('datafono'));
        const esEfectivoCambio = !esTarjeta && (metodoLower.includes('cambio') || metodoLower === 'efectivo_cambio');
        const esEfectivoExacto = !esTarjeta && !esPuntoVenta && !esEfectivoCambio;
        ...
        if (esTarjeta) {
            console.log('Seleccionando método de pago en Ecuador: "Tarjeta" -> "Débito / Crédito"...');
            ...
            for (let intento = 1; intento <= 6; intento++) {
                ...
                if (btnNuevaPresente) {
                    console.log('✅ Sub-radio "Débito / Crédito" seleccionado correctamente en Ecuador.');
                    break;
                } else {
        ...
        if (!esPuntoVenta) {
            ...
            if (esEfectivoExacto) {
                ...
                let montoADar = (typeof montoCambioOCard === 'string' || typeof montoCambioOCard === 'number') ? String(montoCambioOCard) : '50';
    ...
    async procesarPagoYConfirmarOrden() {
        console.log("Iniciando proceso de pago y creación de orden en Ecuador...");
        ...
        console.log("Haciendo clic en el botón 'Pagar' en Ecuador...");
        ...
        console.log("Esperando a que la pantalla cargue el Detalle de la Orden y el Código del Pedido en Ecuador...");
        ...
        for (let seg = 1; seg <= 35; seg++) {
            ...
            // Verificar si el texto del body ya contiene el código
        ...
        console.log("Extrayendo código de pedido de la pantalla de detalle / respuesta...");
