...
export class RegisterPage {
    ...
    constructor(page) {
        ...
        this.botonContinuarCorreo = this.dialogAuth.getByRole('button', { name: /continuar con correo electrónico|continuar con correo|correo electrónico|continuar com e-mail|e-mail/i })
            .or(this.dialogAuth.locator('button, [role="button"], a').filter({ hasText: /continuar con correo electrónico|correo electrónico|continuar com e-mail|e-mail/i }))
            .or(page.getByRole('button', { name: /continuar con correo electrónico|continuar com e-mail/i }));
        ...
        this.inputEmail = this.dialogAuth.locator('input[type="email"], input[name="email"], input#email, input[placeholder*="email"], input[placeholder*="correo"]')
            .or(this.dialogAuth.getByPlaceholder(/hola@email\.com|seu@email\.com/i))
            .or(page.getByPlaceholder('hola@email.com'));

        // Botón Continuar (Paso 1 Correo)
        this.botonContinuarModal = this.dialogAuth.locator('button').filter({ hasText: /^continuar$|^avançar$/i })
            .or(this.dialogAuth.getByRole('button', { name: /^continuar$|^avançar$/i }));
        ...
        this.inputNombre = this.dialogAuth.locator('#name, #firstName, input[name="name"], input[name="firstName"]')
            .or(this.dialogAuth.getByPlaceholder(/nombre|primeiro nome|nome/i));

        this.inputApellido = this.dialogAuth.locator('#lastName, #surName, input[name="lastName"], input[name="surname"]')
            .or(this.dialogAuth.getByPlaceholder(/apellido|sobrenome/i));

        this.inputTelefono = this.dialogAuth.locator('#phone, input[type="tel"], input[name="phone"], input[name="telephone"]')
            .or(this.dialogAuth.getByPlaceholder(/teléfono|celular|telefone/i));

        this.inputDocumento = this.dialogAuth.locator('#document, #dni, #ci, #cedula, #rut, #cpf, input[name="document"], input[name="rut"], input[name="cpf"]')
            .or(this.dialogAuth.getByPlaceholder(/cédula|documento|rut|cpf|dni/i));

        // Checkboxes obligatorios
        this.checkboxesTerminos = this.dialogAuth.locator('input[type="checkbox"]');

        // Botón estricto 'Crear cuenta' dentro del modal
        this.botonCrearCuenta = this.dialogAuth.locator('button').filter({ hasText: /^crear cuenta$|^criar conta$|^registrarse$|^cadastrar$/i })
            .or(this.dialogAuth.getByRole('button', { name: /^crear cuenta$|^criar conta$|^registrarse$|^cadastrar$/i }))
            .or(this.dialogAuth.locator('button[type="submit"]'));

        // Tarjeta / Pantalla roja final de Bienvenida ("¡BIENVENIDO(A) [NOMBRE]!" / "BEM-VINDO(A)")
        this.modalBienvenida = page.locator('.Modal, [class*="Modal"], div').filter({ hasText: /¡BIENVENIDO|BIENVENID|BEM-VINDO/i })
            .or(page.getByText(/¡BIENVENIDO|BEM-VINDO/i));
    }
    ...
    async completarPerfilPostOtp(datos) {
        ...
        // 4. Cédula / Documento / RUT / CPF / DNI
        if (await inputDocumento.isVisible({ timeout: 3000 }).catch(() => false)) {
            console.log(`Llenando Documento: "${datos.documento}"`);
