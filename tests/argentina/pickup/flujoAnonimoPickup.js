...
/**
 * Función base que ejecuta el flujo completo de Pickup en Argentina con el método de pago especificado
 * @param {import('@playwright/test').Page} page
 * @param {import('@playwright/test').TestInfo} testInfo
 * @param {'Tarjeta Débito / Crédito' | 'Efectivo'} metodoPago
 */
async function ejecutarFlujoPickupArgentina(page, testInfo, metodoPago) {
    ...
    await ejecutarPaso(page, testInfo, {
        ...
        descripcion: 'Ingreso de datos personales del cliente en Argentina'
    }, async () => {
    ...
    await ejecutarPaso(page, testInfo, {
        ...
        descripcion: `Selección de '${metodoPago}', procesamiento de pago y captura de número de orden en Argentina`
    }, async () => {
    ...
    console.log(`✅ Flujo Argentina Pickup (${metodoPago}) finalizado exitosamente. Orden: [ ${codigoPedidoGenerado} ]`);
}

// =========================================================================
// 🇦🇷 Casos de Prueba E2E - Métodos de Pago Argentina Pickup
// =========================================================================

test('Flujo E2E - Compra Pickup con Tarjeta Débito/Crédito (Argentina)', async ({ page }, testInfo) => {
    await ejecutarFlujoPickupArgentina(page, testInfo, testData.paymentMethods.tarjeta);
});

test('Flujo E2E - Compra Pickup con Efectivo (Argentina)', async ({ page }, testInfo) => {
    await ejecutarFlujoPickupArgentina(page, testInfo, testData.paymentMethods.efectivo);
});
